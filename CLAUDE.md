...
**Current Feature**: `specs/019-admin-dashboard/plan.md` (admin dashboard overhaul —
replaces the bare two-page admin panel with a new `src/modules/admin/` module covering
user search & detail with a credit-grant action, revenue/MRR & subscriptions, submission
pipeline & grading health, and a growth snapshot (24h/7d/30d/all-time); every admin action
recorded in a new `AdminActionLog` table; v1 is snapshot totals, no charts; retires
`weekly.getAppMetrics` and `/admin/metricas` in favor of the new `/admin` overview;
contract in `contracts/api.md`, types in `data-model.md`, setup in `quickstart.md`).

**Previous Context**: `specs/013-eye-blink-splash/plan.md` (login-screen brand intro — a
...
`specs/009-student-groups/plan.md` (student-led groups — any student creates a group and
becomes its leader, others join via invite code/link (cap 30 participants/group, 5
groups/student as member, unlimited as leader); leader proposes one essay theme at a time,
mirroring the global Redação da Semana theme but scoped to the group; new `groups` module
and 5 Prisma models). `specs/005-add-pdf-support/plan.md` (single-page PDF essay uploads via
