// forked from design-sync lib/source-kit.mjs — argos-enem's src/components/
// holds app-specific feature components (LogoutButton, RenewalBanner,
// AnnotatedText, CreditBalance) alongside the 14 real design-system
// components. componentSrcMap:null excludes them from the component
// name-list/preview cards, but the upstream synth-entry `comps` filter
// (used to write .pkg-entry.mjs) ignores componentSrcMap entirely, so those
// 4 files' `next/link`/`next/navigation`/next-auth imports were getting
// bundled anyway — pulling in `process.env.*` references that ReferenceError
// at runtime in a browser (no `process` global) and broke every component's
// export. This fork makes the synth-entry filter honor componentSrcMap:null
// by basename, same as the name-list already does.
//
...
export async function resolvePackage(ctx) {
  ...
  if (!entry) {
    ...
    const comps = srcFiles.filter((p) => {
      ...
      const base = basename(p).replace(SRC_IMPL_RX, '');
      return srcMap[base] !== null;
    });
